...
void loop() {  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  //************************* если сменили направление, перейдем на следующий шаг
  if (RightComb[step] == encoder) {

    DF_play(1);
    //!!!!!!!!!!!!!!!!!!!!!!!!!!! после звука проверяем, что не превысили зазор (2 шага энкодера)
    while (encoder >= encoderOld && encoder <= encoderOld + 4) {
      GetEncoder();
    }
    if (dirOld != dir) {
      step++;
      //*********************если очередное правильное значение RightComb[N]==0 значит прошли все. Победа!
...
int SetNewComb() {
  ...
  while (digitalRead(BUT_PIN) == 1) {
    ...
    //************************* если сменили нправление, перейдем на следующий шаг
    encoderOld = encoder;
    dirOld = dir;
    //resetFunc();
  }
...
void GetEncoder() {
  ...
  if (encA == 1 && encB == 1 && encAOld == 0 && encBOld == 1) {
    encoder++;
    dir = 0;
    //Serial.print("Dir = "); Serial.print(dir); Serial.print(";  encoder = "); Serial.println(encoder);
  }
...
void setBuff(uint8_t _command, uint16_t _option) {
  //static uint8_t buffer[10] = {0x7E, 0xFF, 0x06, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0xEF};
  uint16_t crc = 0x00;
